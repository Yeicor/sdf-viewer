...
use eframe::egui::{Context, Frame, ProgressBar, ScrollArea, Ui, Vec2, ThemePreference};
use eframe::egui;
...
impl SDFViewerApp {
    ...
    pub fn new(cc: &eframe::CreationContext<'_>, cli_args: CliApp) -> Self {
        ...
        if env_get("light").is_some() { // TODO: Save & restore theme settings
            cc.egui_ctx.set_theme(ThemePreference::Light);
        } else if env_get("dark").is_some() {
